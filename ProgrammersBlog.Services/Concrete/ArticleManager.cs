...
namespace ProgrammersBlog.Services.Concrete
{
    public class ArticleManager : IArticleService
    {
        ...
        public async Task<IResult> Add(ArticleAddDto articleAddDto, string cretadByName)
        {
            var article = _mapper.Map<Article>(articleAddDto);
            article.CreatedByName = cretadByName;
            article.ModifiedByName = cretadByName;
            article.UserId = 1;
        ...
        {
            var articles = await _unitOfWork.Articles.GetAllAysnc(null, a => a.User, a => a.Category);
            if (articles.Count>-1)
            {
        ...
        public async Task<IDataResult<ArticleListDto>> GetAllByCategory(int categoryId)
        {
            ...
            if (result)
            {
                var articles = await _unitOfWork.Articles.GetAllAysnc(a => a.CategoryId == categoryId && !a.IsDeleted && a.IsActive, a => a.User, a => a.Category);
                if (articles.Count > -1)
                {
                    return new DataResult<ArticleListDto>(ResultStatus.Success, new ArticleListDto
                    {
        ...
        {
            var articles = await _unitOfWork.Articles.GetAllAysnc(a => !a.IsDeleted, a => a.User, a => a.Category);
            if (articles.Count>-1)
            {
                return new DataResult<ArticleListDto>(ResultStatus.Success, new ArticleListDto
                {
        ...
        {
            var articles = await _unitOfWork.Articles.GetAllAysnc(a => !a.IsDeleted && a.IsActive, a => a.User, a => a.Category);
            if (articles.Count>-1)
            {
                return new DataResult<ArticleListDto>(ResultStatus.Success, new ArticleListDto
                {
